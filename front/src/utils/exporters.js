...
export const exportToExcel = ({ filename, title = 'AeroPulse Report', summary = {}, rows = [] }) => {
  const normalizedRows = Array.isArray(rows) ? rows : [];
  const headers = normalizedRows.length ? Object.keys(normalizedRows[0]) : [];
  const summaryRows = Object.entries(summary || {})
    .map(([key, value]) => `<Row><Cell ss:StyleID="label"><Data ss:Type="String">${escapeXml(key)}</Data></Cell><Cell><Data ss:Type="String">${escapeXml(value)}</Data></Cell></Row>`)
    .join('');
  const headerCells = headers.map((header) => `<Cell ss:StyleID="header"><Data ss:Type="String">${escapeXml(header)}</Data></Cell>`).join('');
  const dataRows = normalizedRows.map((row) => `<Row>${headers.map((header) => `<Cell><Data ss:Type="String">${escapeXml(row[header])}</Data></Cell>`).join('')}</Row>`).join('');
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
  ...
  <Worksheet ss:Name="Report"><Table>
    ...
    <Row ss:Height="25"><Cell ss:StyleID="title" ss:MergeAcross="4"><Data ss:Type="String">${escapeXml(title)}</Data></Cell></Row>
    <Row><Cell ss:StyleID="subtitle" ss:MergeAcross="4"><Data ss:Type="String">Generated ${escapeXml(new Date().toLocaleString())}</Data></Cell></Row>
    <Row/>
    ${summaryRows ? `<Row><Cell ss:StyleID="header" ss:MergeAcross="1"><Data ss:Type="String">REPORT SUMMARY</Data></Cell></Row>${summaryRows}<Row/>` : ''}
    <Row>${headerCells}</Row>
    ${dataRows}
  </Table></Worksheet>
</Workbook>`;
