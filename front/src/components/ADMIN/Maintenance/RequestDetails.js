...
const SERVICE_QUOTA_REQUIRED_MESSAGE = 'A Service Quota is required before this technician can be assigned to a warranty claim.';
...
const RequestDetails = ({ request, onUpdated }) => {
  ...
  useEffect(() => {
    setScheduledDate(dateValue(current?.scheduledDate || linkedTask?.scheduledDate || current?.preferredDate));
  ...
  const hasCheckIn = Boolean(checkIn?.checkedInAt) && Number.isFinite(checkIn?.latitude) && Number.isFinite(checkIn?.longitude);
  ...
  const assignTechnician = () => {
    ...
    if (current?.warrantyClaimId && (!Number.isSafeInteger(Number(technician.serviceQuota)) || Number(technician.serviceQuota) < 1)) {
      setMessage({ type: 'error', text: SERVICE_QUOTA_REQUIRED_MESSAGE });
      return;
    }
  ...
  return <aside className="maintenance-details" aria-live="polite">
    ...
    <VisitFollowUpPanel task={linkedTask} onUpdated={async () => {
      ...
      setCurrent(value => ({ ...value, scheduledDate: result.task.scheduledDate, timeSlot: result.task.timeSlot }));
      onUpdated?.({ ...current, scheduledDate: result.task.scheduledDate, timeSlot: result.task.timeSlot });
    }} />
    <div className="maintenance-completion-note"><strong>Completion workflow</strong><span>{completionNote}</span></div>
    <div className="maintenance-detail-section"><h3>Technician arrival</h3>{hasCheckIn ? <><p>Checked in: {formatDateTime(checkIn.checkedInAt)}</p><p>{checkIn.latitude}, {checkIn.longitude}</p><a href={`https://www.google.com/maps?q=${checkIn.latitude},${checkIn.longitude}`} target="_blank" rel="noreferrer">Open check-in map</a></> : <p>No GPS check-in recorded yet.</p>}</div>
    {hasProof ? <div className="maintenance-detail-section"><h3>Technician proof of work</h3><dl className="maintenance-detail-grid"><div><dt>Before condition</dt><dd>{linkedTask.beforeCondition || 'Not recorded'}</dd></div><div><dt>After condition</dt><dd>{linkedTask.afterCondition || 'Not recorded'}</dd></div><div><dt>Findings</dt><dd>{linkedTask.findings || 'Not recorded'}</dd></div><div><dt>Resolution</dt><dd>{linkedTask.resolution || 'Not recorded'}</dd></div><div><dt>Submitted</dt><dd>{formatDateTime(proof.submittedAt || linkedTask.proofSubmittedAt)}</dd></div></dl><div className="maintenance-proof-images">{[...(proof.beforePhotos || []).slice(0, 1), ...(proof.afterPhotos || []).slice(0, 1)].map((photo, index) => <a key={`${photo.uri}-${index}`} href={photo.uri} target="_blank" rel="noreferrer"><img src={photo.uri} alt={photo.label || 'Service proof'} /><span>{photo.label || 'Service proof'}</span></a>)}</div></div> : null}
