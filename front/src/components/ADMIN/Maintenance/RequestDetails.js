...
const RequestDetails = ({ request, onUpdated }) => {
  ...
  return <aside className="maintenance-details" aria-live="polite">
    ...
    {!isClosed ? <div className="maintenance-detail-section"><h3>Service appointment</h3><p>Choose the appointment before assigning or reassigning a technician. To reschedule the same technician, update these fields and save the assignment.</p><label className="maintenance-assignment-field"><span>Appointment date</span><input type="date" min={today()} value={scheduledDate} onChange={event => setScheduledDate(event.target.value)} disabled={busy} /></label><label className="maintenance-assignment-field"><span>Time slot</span><input type="text" placeholder="e.g. 9:00 AM – 12:00 PM" maxLength={80} value={timeSlot} onChange={event => setTimeSlot(event.target.value)} disabled={busy} /></label></div> : null}
