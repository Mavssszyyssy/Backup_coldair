      ...
      {!activeClaims.length ? <div className="maintenance-empty"><strong>No warranty claims awaiting review</strong><span>New customer claims will appear here.</span></div> : (
        <div className="maintenance-request-list">
          {activeClaims.map((claim) => {
            ...
            const componentKey = `${claim.unitId}:${claim.claimId}`;
            const coverage = claim.componentCoverage || [];
            return <div className="maintenance-request-item" key={`${claim.unitId}:${claim.claimId}`}>
              <span className="maintenance-request-main">
                ...
                {claim.coverageSummary ? <small>{claim.coverageSummary}</small> : null}
                {coverage.length > 0 ? <label>Component covered
                  <select aria-label={`Component covered for ${claim.claimId}`} value={components[componentKey] || ''} onChange={(event) => setComponents((previous) => ({ ...previous, [componentKey]: event.target.value }))} disabled={busy}>
                    <option value="">Select component</option>
                    {coverage.map((item) => <option key={item.component} value={item.component.toLowerCase()} disabled={item.status !== 'active'}>{item.component} — {readable(item.status)}</option>)}
                  </select>
                </label> : null}
              </span>
              <span className="maintenance-request-meta">
                ...
                <div className="maintenance-action-row">
                  ...
                  <button type="button" className="maintenance-button" disabled={busy || (coverage.length > 0 && !coverage.some((item) => item.component.toLowerCase() === components[componentKey] && item.status === 'active'))} onClick={() => onReview(claim, 'approved', components[componentKey] || '')}>{busy ? 'Saving…' : 'Approve repair'}</button>
