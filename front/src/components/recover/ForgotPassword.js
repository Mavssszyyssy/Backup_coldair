...
function ForgotPassword() {
  ...
  return <BoutiqueAuthLayout>
    ...
    {step === 'request' ? <BoutiqueStack tag="form" gap={22} onSubmit={requestCode} className="bq-fade-in">
      ...
      {usesSharedDemoEmail ? <BoutiqueInput label="Demo Account Login ID" icon={Key} type="text" autoComplete="username" value={accountLoginId} onChange={(event) => { setAccountLoginId(event.target.value); setError(''); setErrorField(''); }} placeholder="admin.cavite or tech.main" hint="Required because this demo inbox belongs to six separate accounts." status={errorField === 'accountLoginId' ? 'error' : null} errorMessage={errorField === 'accountLoginId' ? error : ''} disabled={loading} /> : null}
