...
const SuperAdminBranches = () => {
  ...
  return <SuperAdminLayout title="Branch Management" subtitle="Assign one accountable administrator to each operating branch">
    ...
    <section className="branch-directory branch-email-directory" aria-label="Administrator email management"><header><div><p>Administrator accounts</p><h2>Contact email addresses</h2><span>Editing an email does not change the administrator's account ID, branch, permissions, password, or session ownership.</span></div></header>{loading ? <p className="branch-empty">Loading administrator accounts…</p> : <div>{admins.map((admin) => <article key={admin.id}><div><strong>{admin.name || 'Administrator'}</strong><span>{admin.assignedBranch || 'Unassigned'}</span></div><label>Email address<input type="email" value={admin.email || ''} onChange={(event) => setAdmins((current) => current.map((item) => item.id === admin.id ? { ...item, email: event.target.value } : item))} placeholder="name@example.com" disabled={savingEmailId === admin.id} /></label><button type="button" onClick={() => saveAdminEmail(admin)} disabled={Boolean(savingEmailId)}>{savingEmailId === admin.id ? 'Saving…' : 'Save email'}</button></article>)}</div>}</section>
  </SuperAdminLayout>;
};
