...
const SuperAdminProfile = () => {
  ...
  return <SuperAdminLayout title="My SuperAdmin Profile" subtitle="Personal account details and security for HQ access">
    ...
    <div className="hq-account-grid hq-profile-grid">
      <form className="hq-account-card" onSubmit={saveProfile}><div className="hq-card-heading"><p>Personal details</p><h3>Profile information</h3></div><label>Full name<input value={profile.name} onChange={(event) => setProfile((current) => ({ ...current, name: event.target.value }))} placeholder="Full name" /></label><label>Email address<input type="email" value={profile.email} onChange={(event) => setProfile((current) => ({ ...current, email: event.target.value }))} placeholder="name@example.com" required /><small>This changes your contact email only. Your account ID, role, sessions, and authenticator remain separate.</small></label><label>Mobile number<input value={profile.phone} onChange={(event) => setProfile((current) => ({ ...current, phone: event.target.value }))} placeholder="09XXXXXXXXX" inputMode="tel" /></label><button type="submit" disabled={savingProfile}>{savingProfile ? 'Saving…' : 'Save profile'}</button>{profileNotice ? <span className="hq-success" role="status">{profileNotice}</span> : null}</form>
