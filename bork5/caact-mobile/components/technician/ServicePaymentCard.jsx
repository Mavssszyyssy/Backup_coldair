...
export default function ServicePaymentCard({ task, onUpdated }) {
  ...
  return <Card>
    ...
    {payment.status === "due" && !closed ? <TechButton title="Confirm service cash collected" loading={busy} disabled={!checkedIn || !serviceNoteSaved || busy} onPress={confirm} /> : null}
    {payment.status === "due" && !closed && !checkedIn ? <Text>GPS check-in is required before cash collection.</Text> : null}
    {payment.status === "due" && !closed && checkedIn && !serviceNoteSaved ? <Text>Save the service note and any labor or parts costs before collecting payment.</Text> : null}
  </Card>;
}
