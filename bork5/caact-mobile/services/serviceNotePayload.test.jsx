...
test("dropdown sentences reach the API as individual actions with the existing report format", async () => {
  await upsertServiceLog({ taskId: "task1", logType: "deep_cleaning", technicianStatus: "for_further_inspection", findings: "Dust buildup on the evaporator coil.", resolution: "Removed and disassembled the indoor unit for deep cleaning.\nCleaned the evaporator coil.", notes: "Customer advice." });
  expect(api.patchTask).toHaveBeenCalledWith("test-session", "task1", expect.objectContaining({ serviceType: "deep_cleaning", technicianStatus: "for_further_inspection", findings: "Dust buildup on the evaporator coil.", serviceActions: ["Removed and disassembled the indoor unit for deep cleaning.", "Cleaned the evaporator coil."], notes: "Customer advice." }));
});
