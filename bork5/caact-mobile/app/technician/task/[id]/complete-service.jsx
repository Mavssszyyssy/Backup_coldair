  ...
  const formError = installationTask
    ? !canComplete
      ...
      : !afterPhotos.some((photo) => photo?.uri)
        ...
        : ""
    : choiceError || serviceReportError(findings, resolution) || (!afterPhotos.some(photo => photo?.uri) ? "Capture an after-service photo before completing this visit." : "") || (task?.servicePayment?.status === "quote_required" ? "Ask Admin to set the service quote first." : task?.servicePayment?.status === "due" ? "Confirm service cash collection before completion." : "");
