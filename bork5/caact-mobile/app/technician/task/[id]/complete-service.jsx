...
export default function CompleteServiceScreen() {
  ...
  const formError = !canComplete ? "Scan and verify every assigned AC unit before completing this installation." : !afterPhotos.some((photo) => photo?.uri) ? "Capture the installed AC unit as proof." : "";
  ...
  const submit = async () => {
    ...
    const confirmed = await new Promise((resolve) => Alert.alert("Complete installation?", "This sends the verified QR record and installed-unit photo to the customer order.", [{ text: "Cancel", style: "cancel", onPress: () => resolve(false) }, { text: "Complete", onPress: () => resolve(true) }]));
    if (!confirmed) return;
